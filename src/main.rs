...
fn main() -> std::io::Result<()> {
    let matches = App::new("udp_forwarder")
        ...
        .arg(Arg::with_name("input-address").short("i").help("The address the server should listen on").default_value("0.0.0.0:0").required(false).takes_value(true))
    ...
    let is_sending = matches.values_of("sending").is_some();
    ...
    if is_async {
        ...
        let send_address = peers.get(0).unwrap();
        result = fudp::sender::run(listen_address, *send_address);
    } else {
