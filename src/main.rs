...
fn main() -> std::io::Result<()> {
    let matches = App::new("udp_forwarder")
        ...
        .arg(Arg::with_name("input-address").short("i").help("The address the server should listen on").default_value("").required(false).takes_value(true))
        ...
        .arg(Arg::with_name("async").short("a").help("Should the server handle udp async or not"))
        .arg(Arg::with_name("no-queue").short("q").help("Disables queueing in blocking mode"))
        .arg(Arg::with_name("sending").short("s").help("Switches to sending mode"))
        .arg(Arg::with_name("packet-size").short("pz").takes_value(true).default_value("300").help("Set's the send packet size"))
        .get_matches();
    ...
    let result;
    if is_sending {
        result = fudp::sender::run(listen_address, &peers, send_packet_size);
    } else if is_async {
